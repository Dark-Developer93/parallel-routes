    ...
    <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
      ...
      <div className="p-4">
          ...
          <div className="flex -space-x-2">
            {data.members.map((member) => (
              <div
                ...
                } rounded-full border-2 border-white flex items-center justify-center text-white text-xs font-medium`}
              >
                {member.initials}
              </div>
            ))}
        ...
        <ul className="space-y-3">
          {data.members.map((member) => (
            <Link href={`/memberDetails/${member.id}`} key={member.id}>
                  ...
                  <div className="relative">
                    <div
                      className={`w-12 h-12 bg-gradient-to-br ${
                        gradientClasses[member.gradientFrom.split("-")[0]]
                      } rounded-xl flex items-center justify-center text-white font-semibold text-lg shadow-sm group-hover:shadow-md transition-shadow`}
                    >
                      {member.initials}
                    </div>
                    <div
